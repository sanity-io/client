 ...
 */
import {type HttpRequest, requester as originalRequester, type RequestOptions} from '@sanity/client'
import isPlainObject from 'lodash.isplainobject'

import {createTranscoder} from '../csm/transcode'
import type {ContentSourceMapQueryResponse, PreviewKitClientConfig} from './types'
...
function transcodeResponse({studioUrl, encodeSourceMapAtPath, logger}: TranscodeResponseConfig) {
      ...
      if (
        ...
        if (logger) {
          ...
          if (transcoderResult.report.skipped.length > 0) {
            ...
            logger?.log?.(`[@sanity/preview-kit]: List of skipped paths`, [...skipped.values()])
          }
...
export function createHttpRequest({
  ...
  encodeSourceMapAtPath,
  logger,
}: Pick<PreviewKitClientConfig, 'studioUrl' | 'encodeSourceMapAtPath' | 'logger'>): HttpRequest {
  ...
  function httpRequest(options: RequestOptions, requester = superRequester): HttpRequest {
...
/** @alpha */
export function isResultBody(body: unknown): body is ContentSourceMapQueryResponse {
  return typeof body === 'object' && body !== null && 'result' in body
}

/** @alpha */
export function isContentSourceMapBody(body: unknown): body is ContentSourceMapQueryResponse {
