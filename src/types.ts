...
/**
 * Creates a new `system.variant` definition document under the given id.
 *
 * @public
 * @beta
 */
export interface CreateVariantDefinitionAction {
  ...
  /**
   * Conditions used to select this variant. Values cannot contain commas.
   */
  conditions: ClientVariantConditions

  /**
   * Selection priority. Higher values are preferred when multiple variants match.
   */
  priority: number
 ...
 *
 * Only `conditions`, `priority` and `metadata` are mutable; system fields
 * (`_id`, `_type`, `name`, timestamps) are immutable and rejected.
 *
 * @public
 * @beta
 */
