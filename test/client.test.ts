...
describe('client', async () => {
  ...
  describe('mediaLibrary', () => {
    ...
    test('video.getPlaybackInfo with container ID fallback', async () => {
      const client = getClient()
    ...
    test('video.getPlaybackInfo throws error for invalid asset identifier', async () => {
      const client = getClient()
    ...
    test('video.getPlaybackInfo with signed/secured response', async () => {
      ...
      if (tokens) {
        expectTypeOf(result).toMatchTypeOf<VideoPlaybackInfoSigned>()
        expectTypeOf(result.stream.token).toBeString()
        expectTypeOf(result.thumbnail.token).toBeString()
      }
    })
  })
})
