...
describe('fetching documents', () => {
  ...
  test('documentsExists returns set with all ids when none are omitted', async () => {
    getActiveMock()
      .scope(projectHost())
      .on('GET', '/v1/data/doc/foo/abc123,abc321', {
  ...
  test('documentsExists keeps ids omitted with reason other than "existence"', async () => {
    getActiveMock()
      .scope(projectHost())
      .on('GET', '/v1/data/doc/foo/abc123,abc321', {
        query: objectContaining({excludeContent: 'true'}),
  ...
  test('documentsExists returns empty set for empty ids without making a request', async () => {
  ...
  test('documentsExists treats missing omitted field as all ids existing', async () => {
    getActiveMock()
      .scope(projectHost())
      .on('GET', '/v1/data/doc/foo/abc123,abc321', {
        query: objectContaining({excludeContent: 'true'}),
  ...
  test('documentsExists percent-encodes ids so delimiter chars are preserved', async () => {
    getActiveMock()
      .scope(projectHost())
  ...
  test('documentsExists batches large id arrays into requests of at most 100 ids', async () => {
  ...
  test('gives http statuscode as error if no body is present on errors', async () => {
    expect.assertions(2)

    getActiveMock().scope(projectHost()).on('GET', '/v1/data/doc/foo/abc123').respond({status: 400})
  ...
  test('includes part of body if expected JSON object not returned on errors', async () => {
    expect.assertions(2)
  ...
  describe('can cancel request with an abort controller signal', () => {
